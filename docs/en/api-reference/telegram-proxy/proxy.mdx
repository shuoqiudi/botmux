...
An administrator session or API key is required by default. A legacy backend
may omit that credential only when its direct peer address is in
`-tgapi-trusted-cidrs`; forwarded headers are ignored.
...
| Method | Behavior |
|--------|----------|
| `setWebhook` | Configures the bot's backend URL in BotMux and enables proxy mode. Auto-registration of unknown bots via `getMe` requires `BOTMUX_ALLOW_AUTO_REGISTER=1`. Supports `url` and `drop_pending_updates` parameters. |
