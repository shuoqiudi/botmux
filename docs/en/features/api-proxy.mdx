...
`/tgapi/` and `/tgapi/file/` require an admin session or admin API key by
default. A legacy backend that cannot send an admin credential may connect from
an isolated network explicitly listed with `-tgapi-trusted-cidrs`. BotMux checks
the direct peer address and ignores forwarded headers; never trust a public or
