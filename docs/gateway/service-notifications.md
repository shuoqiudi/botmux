...
管理员在服务详情选择 Bot 后，聊天列表会合并该 Bot 已知的聊天与手动登记的目标，并标记已订阅项。点击“刷新聊天”可重新读取列表。只有主动点击订阅时，才会验证并登记尚未配置的聊天目标，然后建立订阅；浏览列表不会登记目标或发送消息。若 Bot 尚未收到该聊天的更新，可先让 Bot 在该聊天收到消息，或通过高级设置手动填写 Chat ID。
